...
function errorsFor(x, runtimeText){
 const e=[]; const fail=(id,message)=>e.push({id,message});
 if(runtimeText){ for(const term of ['Golden Reference','Build Intent Brief','تصویر ذهنی','deterministic','must not invent','must not paraphrase']) if(!text.includes(term)) fail('EV4-RUNTIME-001',`runtime files must mention ${term}.`); }
...
for(const f of process.argv.slice(2)){
 const x=readJson(f); const errs=errorsFor(x,true);
