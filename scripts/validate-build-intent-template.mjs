...
if(!r || typeof r !== 'object' || Array.isArray(r)){
 fail('EV4-BITPL-000','Missing template registry root object.');
} else {
 const seenTemplates = new Set();
 if(!Array.isArray(r.templates)){
  fail('EV4-BITPL-005','templates must be an array.');
 }
 ...
 for(const t of templates){
  ...
  const seen=[...String(t?.template || '').matchAll(/\{([^}]+)\}/g)].map((m)=>m[1]);
