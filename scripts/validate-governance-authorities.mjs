...
for (const [name,statePath,schemaPath] of specs) {
  const state = readYaml(statePath); loaded[name] = state;
...
for (const path of Object.keys(expectedExternalFiles)) if (!prod5Authorities.includes(`${projectGateRepository}@${projectGateCommit}:${path}`)) errors.push(`PROD-CAP-005 missing external path: ${path}`);
...
for (const item of authorityFixture.green_merge_predicate_cases || []) { const value={...lifecycle.green_merge_predicates,...(item.overrides||{})}; if (Object.entries(expectedGreen).every(([key,expected])=>value[key]===expected)) errors.push(`authority fixture ${item.case_id} unexpectedly passed.`); }
...
for (const value of requiredNonClaims) { if (!policy.non_claims?.includes(value)) errors.push(`policy non-claim missing: ${value}.`); if (!plan.prohibited_claims?.includes(value)) errors.push(`plan prohibited claim missing: ${value}.`); }
...
const invalidPatterns = new Map([
 ['The Project Gate program is not implemented.','PG-AUTH-001_EXTERNAL_CAPABILITY_DENIED'],['The external Project Gate verifier remains future work.','PG-AUTH-002_EXTERNAL_VERIFIER_FUTURE_ONLY'],['project_gate_runtime: implemented','PG-AUTH-003_EXTERNAL_LOCAL_CONFLATION'],['name: project_gate_verifier_and_runtime','PG-AUTH-004_AMBIGUOUS_CAPABILITY_IDENTITY'],['real_non_synthetic_ce_to_builder_handoff: proven','PG-AUTH-005_REAL_HANDOFF_OVERCLAIM'],['production_ready: true','PG-AUTH-006_PRODUCTION_READY_OVERCLAIM'],['Project Gate replaces Builder contracts.','PG-AUTH-007_BUILDER_AUTHORITY_REPLACED'],['project_gate_receipt_is_builder_semantic_input: true','PG-AUTH-008_RECEIPT_INPUT_CONFLATION'],['direct_controlled_ce_to_builder_path: unsupported','PG-AUTH-009_DIRECT_CONTROLLED_PATH_REMOVED']
]);
...
for (const [text,diagnostic] of invalidPatterns) for (const [path,content] of Object.entries(carrierText)) if (content.includes(text)) errors.push(`${diagnostic}:${path}`);
...
if (errors.length) { printFailure('Governance authority validation failed:', errors); process.exit(1); }
