...
if (completion.claim_scope === 'desktop' && completion.reference_paradigm_lock_present && states.desktop_reference_paradigm_matched !== true) {
  fail('EV4-CSTATUS-002', 'desktop complete cannot be claimed before reference paradigm is matched.');
}
if (completion.claim_scope === 'section' && !completion.scope_excludes_responsive && (!states.tablet_checked || !states.mobile_checked)) {
  fail('EV4-CSTATUS-003', 'section complete requires tablet/mobile checked unless scope excludes them.');
}
if (completion.claim_scope === 'acceptable' && (!completion.acceptable_scope || !completion.acceptable_checklist)) {
  fail('EV4-CSTATUS-004', 'acceptable must specify scope and checklist.');
}
