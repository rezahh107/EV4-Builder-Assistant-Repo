#!/usr/bin/env python3
"""Thin adapter over the immutable PR Inspector implementation.

No decision, routing, provenance, or evidence-ID logic is implemented here.
The adapter only loads bounded inputs, calls official PR Inspector functions,
and serializes official results.
"""
...
def configure_checkout(root_value: str) -> dict[str, Any]:
    ...
    for name, module in modules.items():
        ...
        if root != module_path and root not in module_path.parents:
            raise ValueError(
                f"{name} was not imported from the immutable checkout"
            )
    ...
    active_version = (
        manifest.get("active_version") if isinstance(manifest, dict) else None
    )
    if current_version != EXPECTED_PROTOCOL:
        raise ValueError(
            f"CURRENT_VERSION must equal {EXPECTED_PROTOCOL}, received "
            f"{current_version}"
        )
    if active_version != EXPECTED_PROTOCOL:
        raise ValueError(
            f"active_version must equal {EXPECTED_PROTOCOL}, received "
            f"{active_version}"
        )
    return {
        "root": str(root),
...
def verify(args: argparse.Namespace) -> None:
    ...
    inspector_commit = verify_github_commit_payload(
        load_object(Path(args.repository_payload)),
        load_object(Path(args.commit_payload)),
    ...
    emit(
        {
            ...
            "official_provenance_boundary": (
                "pr_inspector.review_provenance.verify_review_directory"
            ),
            "official_provenance_function": (
            ...
            "completion": {
                ...
                "target_head_receipt_sha256": (
                    completion.target_head_receipt_sha256
                ),
                "technical_status": completion_projection[
                    "technical_status"
                ],
                "approval_requirement": completion_projection[
                    "approval_requirement"
                ],
                "next_action_kind": completion_projection[
                    "next_action"
                ]["kind"],
            },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verification.add_argument("--repository-payload", required=True)
    verification.add_argument("--commit-payload", required=True)
