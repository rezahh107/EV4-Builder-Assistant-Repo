...
if(!c){
  ...
  for(const inv of c.hard_invariants || []){
  ...
  for(const m of c.visual_tolerance_policy?.metrics || []){
    ...
    for(const zone of ['pass','ambiguous','major']){
      const z = m[zone];
      if(!z || typeof z !== 'object' || Object.keys(z).length === 0 || !Object.values(z).every((v)=>typeof v==='number')) fail('EV4-GREF-005',`metric ${m.metric} requires numeric ${zone} zone.`);
    }
  }
}
