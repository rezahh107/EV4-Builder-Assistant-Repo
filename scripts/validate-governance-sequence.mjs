...
function evaluateScenario(scenario) {
  ...
  const disclosedChanges = [...(disclosure.lifecycle_changes || [])]
    .map(normalizeChange)
    .sort((a, b) => (a.capability_id ?? '').localeCompare(b.capability_id ?? ''));
  const normalizedExpectedChanges = expectedDisclosure.lifecycleChanges
    .map(normalizeChange)
    .sort((a, b) => (a.capability_id ?? '').localeCompare(b.capability_id ?? ''));
  if (
    ...
    JSON.stringify(disclosedChanges) !== JSON.stringify(normalizedExpectedChanges)
  ) {
  ...
  if (!receipt) {
    diagnostics.push('GOV-SEQ-019_REVIEW_RECEIPT_MISSING');
  } else {
    for (const error of validateSchema(RECEIPT_SCHEMA, receipt, '$.review_receipt')) {
      diagnostics.push(`GOV-SEQ-006_REVIEW_RECEIPT_SCHEMA:${error}`);
    }
    if (
      receipt.independent !== true ||
      receipt.implementation_context_id === receipt.reviewer_context_id
    ) {
      diagnostics.push('GOV-SEQ-007_REVIEW_NOT_INDEPENDENT');
    }
    ...
    if (receipt.technical_status === 'GREEN_MERGE_RECOMMENDED') {
      ...
      const allChecksSuccess =
        ...
        requiredChecks.every((name) => checks[name] === 'success') &&
        Object.keys(checks).every((name) => requiredChecks.includes(name));
      if (!allChecksSuccess || (receipt.exact_head_ci_run_ids || []).length === 0) {
      ...
      if (
        ...
        !setEquals(requiredArtifacts, presentArtifacts) ||
        !/^[0-9a-f]{40}$/.test(completion.implementation_commit || '') ||
        !Array.isArray(completion.ci_run_ids) ||
        completion.ci_run_ids.length === 0
      ) {
        diagnostics.push('GOV-SEQ-011_INCOMPLETE_COMPLETION_EVIDENCE');
      }

      if ((receipt.findings || []).some((finding) => finding.blocking === true)) {
        diagnostics.push('GOV-SEQ-012_GREEN_WITH_BLOCKING_FINDING');
      }
    }
  }
  ...
  if (forbiddenHumanFields.length > 0) {
    diagnostics.push(`GOV-SEQ-014_HUMAN_TECHNICAL_GATE:${forbiddenHumanFields.join(',')}`);
  }

  const security = scenario.security || {};
  if (
    security.repository_visibility === 'public' &&
    security.public_repository_disposition !== 'retain_minimum_security_with_public_repository_hygiene'
  ) {
  ...
  if (
    ...
    (!security.destructive_action.exact_target || !security.destructive_action.recovery_path)
  ) {
...
for (const filePath of listJsonFiles('tests/governance/valid')) {
  ...
  for (const entry of document.scenarios || []) {
    ...
    if (diagnostics.length > 0) {
      errors.push(`${filePath}:${entry.scenario_id}: expected valid, got ${JSON.stringify(diagnostics)}.`);
    }
  }
}
...
for (const filePath of listJsonFiles('tests/governance/invalid')) {
  ...
  for (const entry of document.scenarios || []) {
    ...
    for (const code of expected) {
      if (!diagnostics.some((diagnostic) => diagnostic === code || diagnostic.startsWith(`${code}:`))) {
        errors.push(`${filePath}:${entry.scenario_id}: missing expected diagnostic ${code}; got ${JSON.stringify(diagnostics)}.`);
      }
    }
    if (diagnostics.length === 0) {
      errors.push(`${filePath}:${entry.scenario_id}: invalid fixture unexpectedly passed.`);
    }
  }
}
