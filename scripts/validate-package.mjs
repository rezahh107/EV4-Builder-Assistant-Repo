...
function sortedCanonicalJson(value) {
  if (Array.isArray(value)) {
    return `[${value.map(sortedCanonicalJson).join(',')}]`;
  }

  if (value && typeof value === 'object') {
    return `{${Object.keys(value)
      .sort()
      .map((key) => `${JSON.stringify(key)}:${sortedCanonicalJson(value[key])}`)
      .join(',')}}`;
  }
...
function packageWithoutDigest(value) {
  ...
  if (cloned.input_authorization?.package_digest) {
    delete cloned.input_authorization.package_digest;
  }
  return cloned;
}
...
function expectedDecision(value, diagnostics) {
  if (value.package_status === 'blocked') {
...
if (pkg.selected_candidate_locked !== true) {
  fail(DIAGNOSTICS.SELECTED_CANDIDATE_NOT_LOCKED, 'selected_candidate_locked must be true.');
}

if (pkg.production_ready_allowed !== false) {
  fail(DIAGNOSTICS.PRODUCTION_READY_NOT_FALSE, 'production_ready_allowed must be false.');
}
...
if (nodes.length === 0) {
  fail(DIAGNOSTICS.MISSING_REQUIRED_TREE, 'approved_structure_tree must contain at least one node.');
}

for (const duplicate of duplicates(nodeIds)) {
  fail(DIAGNOSTICS.DUPLICATE_ID, `Duplicate node_id: ${duplicate}`);
}

for (const node of nodes) {
  for (const child of node.children || []) {
    if (!nodeIdSet.has(child)) {
      fail(DIAGNOSTICS.CHILD_NODE_UNKNOWN, `Node ${node.node_id} references unknown child node_id: ${child}`);
    }
  }

  if (!node.element_generation || !node.element_generation_source) {
    fail(
      DIAGNOSTICS.MISSING_GENERATION_EVIDENCE,
      `Node ${node.node_id || '<missing-node-id>'} lacks element_generation or element_generation_source.`
    );
    continue;
  }

  if (node.element_generation === 'Unverified element type' && node.element_generation_source !== 'unverified') {
    fail(DIAGNOSTICS.GENERATION_SOURCE_MISMATCH, `Node ${node.node_id} uses Unverified element type but source is not unverified.`);
  }

  if (node.element_generation !== 'Unverified element type' && node.element_generation_source === 'unverified') {
    fail(DIAGNOSTICS.GENERATION_SOURCE_MISMATCH, `Node ${node.node_id} has verified generation label but unverified source.`);
  }
}
...
if (classMap.length === 0) {
  fail(DIAGNOSTICS.MISSING_CLASS_MAP, 'class_creation_application_map must contain at least one class mapping.');
}
...
for (const entry of classMap) {
  ...
  if (typeof target === 'string' && target.startsWith('n-') && !nodeIdSet.has(target)) {
    fail(DIAGNOSTICS.CLASS_TARGET_UNKNOWN, `Class ${entry.class_name} maps to unknown node_id: ${target}`);
  }
}

for (const widget of pkg.widget_mapping_table || []) {
  if (widget.class_name && !classNames.has(widget.class_name)) {
    fail(DIAGNOSTICS.CLASS_REFERENCE_UNKNOWN, `Widget mapping references class not present in class map: ${widget.class_name}`);
  }
}
...
if (actions.length === 0) {
  fail(DIAGNOSTICS.MISSING_FIRST_BATCH_ACTIONS, 'first_builder_batch.actions must contain at least one action.');
}

for (const duplicate of duplicates(actionIds)) {
  fail(DIAGNOSTICS.DUPLICATE_ID, `Duplicate action_id: ${duplicate}`);
}
...
for (const action of actions) {
  ...
  if (!targetKnown) {
    fail(DIAGNOSTICS.ACTION_TARGET_UNKNOWN, `Action ${action.action_id} targets unknown element: ${target}`);
  }

  if (action.active_class && !classNames.has(action.active_class)) {
    fail(DIAGNOSTICS.CLASS_REFERENCE_UNKNOWN, `Action ${action.action_id} references class not present in class map: ${action.active_class}`);
  }

  if (!action.element_generation || !action.element_generation_source) {
    fail(
      DIAGNOSTICS.MISSING_GENERATION_EVIDENCE,
      `Action ${action.action_id || '<missing-action-id>'} lacks element_generation or element_generation_source.`
    );
    continue;
  }

  if (action.element_generation === 'Unverified element type' && action.element_generation_source !== 'unverified') {
    fail(DIAGNOSTICS.GENERATION_SOURCE_MISMATCH, `Action ${action.action_id} uses Unverified element type but source is not unverified.`);
  }

  if (action.element_generation !== 'Unverified element type' && action.element_generation_source === 'unverified') {
    fail(DIAGNOSTICS.GENERATION_SOURCE_MISMATCH, `Action ${action.action_id} has verified generation label but unverified source.`);
  }
}
...
if (pkg.input_authorization) {
  ...
  if (actual.decision !== expected.decision) {
    fail(
      DIAGNOSTICS.INPUT_AUTHORIZATION_MISMATCH,
      `input_authorization.decision must be ${expected.decision}; received ${actual.decision}.`,
      { blocking: false }
    );
  }

  if (actual.eligible_workflow_mode !== expected.eligible_workflow_mode) {
    fail(
      DIAGNOSTICS.INPUT_AUTHORIZATION_MISMATCH,
      `input_authorization.eligible_workflow_mode must be ${expected.eligible_workflow_mode}; received ${actual.eligible_workflow_mode}.`,
      { blocking: false }
    );
  }

  if (actual.eligible_runtime_state !== expected.eligible_runtime_state) {
    fail(
      DIAGNOSTICS.INPUT_AUTHORIZATION_MISMATCH,
      `input_authorization.eligible_runtime_state must be ${expected.eligible_runtime_state}; received ${actual.eligible_runtime_state}.`,
      { blocking: false }
    );
  }

  if (!arraysEqual(actual.blocking_diagnostics, expected.blocking_diagnostics)) {
    fail(
      DIAGNOSTICS.INPUT_AUTHORIZATION_MISMATCH,
      `input_authorization.blocking_diagnostics must be ${JSON.stringify(expected.blocking_diagnostics)}; received ${JSON.stringify(actual.blocking_diagnostics)}.`,
      { blocking: false }
    );
  }

  if (!arraysEqual(actual.visible_flags, expected.visible_flags)) {
    fail(
      DIAGNOSTICS.INPUT_AUTHORIZATION_MISMATCH,
      `input_authorization.visible_flags must be ${JSON.stringify(expected.visible_flags)}; received ${JSON.stringify(actual.visible_flags)}.`,
      { blocking: false }
    );
  }

  if (actual.package_digest?.value !== expected.package_digest.value) {
    fail(
      DIAGNOSTICS.PACKAGE_DIGEST_MISMATCH,
      `input_authorization.package_digest.value must be ${expected.package_digest.value}; received ${actual.package_digest?.value}.`,
      { blocking: false }
    );
  }
}
