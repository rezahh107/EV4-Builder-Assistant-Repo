...
function requireObject(value, location) { if (!isObject(value)) throw new Error(`${location} must be an object.`); return value; }
function requireString(value, location) { if (typeof value !== 'string' || value.trim().length === 0) throw new Error(`${location} must be a non-empty string.`); return value.trim(); }
function requireArray(value, location) { if (!Array.isArray(value) || value.length === 0) throw new Error(`${location} must be a non-empty array.`); return value; }
function requireEmptyArray(value, location) { if (!Array.isArray(value) || value.length !== 0) throw new Error(`${location} must be an empty array.`); }
...
function sortedCanonicalJson(value) { if (Array.isArray(value)) return `[${value.map(sortedCanonicalJson).join(',')}]`; if (value && typeof value === 'object') return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${sortedCanonicalJson(value[key])}`).join(',')}}`; return JSON.stringify(value); }
function packageWithoutDigest(value) { const cloned = JSON.parse(JSON.stringify(value)); if (cloned.input_authorization?.package_digest) delete cloned.input_authorization.package_digest; return cloned; }
function computePackageDigest(value) { return crypto.createHash('sha256').update(sortedCanonicalJson(packageWithoutDigest(value)), 'utf8').digest('hex'); }
...
function batchPrefixFromActionId(actionId) { const match = actionId.match(/^(BATCH-[A-Z0-9]+)-A\d+$/); return match ? match[1] : null; }
