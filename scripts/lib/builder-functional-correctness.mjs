...
function inactiveLegacyAuthority(operation) {
  return {
    ...
    diagnostics: [{
      ...
      message: `${operation} is not an active real Runtime authority. Use the canonical stable-Run-directory API.`
    }]
  };
}
...
export function writeStrictRealIntake() { return inactiveLegacyAuthority('writeStrictRealIntake'); }
export function validateEmitBatchTransaction() { return inactiveLegacyAuthority('validateEmitBatchTransaction'); }
export function publishEmitBatchTransaction() { return inactiveLegacyAuthority('publishEmitBatchTransaction'); }
export function validateConfirmationTransaction() { return inactiveLegacyAuthority('validateConfirmationTransaction'); }
export function publishConfirmationTransaction() { return inactiveLegacyAuthority('publishConfirmationTransaction'); }
export function validateStrictConfirmationReceipt() { return inactiveLegacyAuthority('validateStrictConfirmationReceipt'); }
export function verifyStrictEvidenceLedger() { return inactiveLegacyAuthority('verifyStrictEvidenceLedger'); }
export function validateStrictRealCompletion() { return inactiveLegacyAuthority('validateStrictRealCompletion'); }
export function publishStrictRealCompletion() { return inactiveLegacyAuthority('publishStrictRealCompletion'); }
...
export function validateCanonicalResume(options) {
  ...
  return {
    ...
    real_run_authority: false,
    passed: result.passed && diagnostics.length === 0,
    diagnostics,
