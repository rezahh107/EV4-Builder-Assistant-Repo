...
const ALLOWED_BY_MODE = {
  START_INTAKE_MODE: new Set(['INTAKE_WAITING', 'INTAKE_VALIDATING', 'EVIDENCE_REQUIRED', 'REVIEW_ONLY', 'CORRECTION', 'PAUSED']),
  ...
  FRESH_IMAGE_MODE_LIMITED: new Set(['INTAKE_WAITING', 'EVIDENCE_REQUIRED', 'BUILD_ACTIVE', 'WAITING_FOR_CONFIRMATION', 'CORRECTION', 'REVIEW_ONLY', 'PAUSED'])
};
...
if (session.runtime_state === 'PAUSED') {
  if (!session.session_id || !session.package_digest || !session.resume_target) {
    ...
    if (!targetStates?.has(session.resume_target.runtime_state) || session.resume_target.runtime_state === 'COMPLETED') {
      fail('EV4-SESSION-012', 'resume_target must be a legal non-COMPLETED state.');
    }
  }
}
...
if (session.runtime_state === 'COMPLETED') {
  if (session.workflow_mode !== 'APPROVED_HANDOFF_MODE') {
    fail('EV4-SESSION-013', 'COMPLETED is allowed only in APPROVED_HANDOFF_MODE.');
  }
  if (!session.session_id || !session.package_digest) {
    fail('EV4-SESSION-014', 'COMPLETED requires session_id and package_digest.');
  }
  if ((session.unresolved_evidence || []).length > 0) {
    fail('EV4-SESSION-015', 'COMPLETED cannot preserve unresolved blocking evidence.');
  }
}

if (checkpoint?.schema === 'ev4-builder-checkpoint@0.2.0') {
  if (session.session_id && checkpoint.session_id && session.session_id !== checkpoint.session_id) {
    fail('EV4-SESSION-016', 'session_id must match last_verified_checkpoint.session_id.');
  }
  if (session.package_digest && checkpoint.package_digest && session.package_digest !== checkpoint.package_digest) {
    fail('EV4-SESSION-017', 'package_digest must match last_verified_checkpoint.package_digest.');
  }
  ...
  for (const blocker of checkpointBlockers) {
    if (!sessionBlockers.includes(blocker)) {
      fail('EV4-SESSION-018', `Unresolved blocker ${blocker} disappeared from Session State.`);
    }
  }
}
